...
### **4. Create ETCD Snapshot Directory**
...
📌 Manual snapshot command for reference, no need to run. Our script will take timely snapshot.
...
```cron
0 0 * * * /usr/bin/python3 /home/kube/script-etcd/etcd.py
```

📌 This runs the snapshot command **everyday at 12AM**.
...
ssh-keygen -t rsa
ssh-copy-id kube@192.168.30.201                     # worker's IP
```
...
- Once the **master-1's** failure is detected this DR (Disaster Recovery) script will be executed.
- Automatically downloads the latest ETCD snapshot from the MinIO bucket.
- Restores the ETCD snapshot on the standby master node.
- Executes kubeadm init on master-2 (standby master) to reinitialize the control plane.
- Connects to the worker nodes via SSH.
- Removes old master-1 join configurations and related Kubernetes directories on the workers.
- Runs the new join command generated by master-2 to reattach the worker nodes to the restored cluster.
...
- This script will monitor **master-1's** API health and it's ping response.
- If the **master-1** goes down for above 5 minutes, this script will initiate the **DR (Disaster Recovery)** scirpt.
